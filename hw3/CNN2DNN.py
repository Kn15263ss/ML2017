...
with open(sys.argv[1]) as trainFile:
  ...
  ratio = 0.8
  xSize = int(x_arr.shape[0]*ratio)
  ySize = int(y_arr.shape[0]*ratio)
  x_train_data = x_arr[:xSize, :]
  y_train_data = y_arr[:ySize, :]

  x_train_data = x_train_data.reshape(x_train_data.shape[0], 48*48).astype(np.float32)#(28709,48,48)
  y_train_data = y_train_data.astype(np.int)#(28709,1)
...
model = Sequential()
model.add(Dense(first_layer * 2, input_shape = (48*48,), name="block1_dense1"))
#model.add(Dense(first_layer * 2, name="block1_dense2"))
...
model.add(Dense(second_layer * 2 , name = "block2_dense1"))
...
model.add(Dense(third_layer * 3 - 150, name="block3_dense1"))
...
model.add(Dense(fourth_layer * 2 + 270 , name="block4_dense1"))
...
#sgd = Nadam(lr=0.0001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.004)
