import numpy as np
import sys
from log_regression_best import LogReg
...
# Read model
try:
  	with open("./model/w_log_best.csv") as model_file:
  		model_list = model_file.read().splitlines()
  		w_model = np.array([[str(item) for item in line.split(",")] for line in model_list])

  	with open("./model/w2_log_best.csv") as model_file:
  		model_list = model_file.read().splitlines()
  		w2_model = np.array([[str(item) for item in line.split(",")] for line in model_list])

  	with open("./model/w3_log_best.csv") as model_file:
  		model_list = model_file.read().splitlines()
  		w3_model = np.array([[str(item) for item in line.split(",")] for line in model_list])

	with open("./model/b_log_best.csv") as model_file:
		model_list = model_file.read().splitlines()
...
predict = LogReg(x_train_data, y_train_data, x_test_data, valid_percent = 0.2, w_model = w_model, w2_model = w2_model, w3_model = w3_model, b_model = b_model)
