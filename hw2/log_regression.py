...
class LogReg:
	def __init__(self, x_train_data, y_train_data, x_test_data, valid_percent, w_model, w2_model, b_model):
	...
	def loss_function(self):
		...
		z_valid = x_valid.dot(self.w)+(x_valid**2).dot(self.w2)+self.b
		z_train = x_train.dot(self.w)+(x_train**2).dot(self.w2)+self.b
	...
	def test_function(self):	
		...
		predict_test = self.test_data_set.dot(self.w)+(self.test_data_set**2).dot(self.w2)+self.b
		...
		np.savetxt("./model/w_log.csv", self.w, delimiter = ",", fmt = "%s")
		np.savetxt("./model/w2_log.csv", self.w2, delimiter = ",", fmt = "%s")
		np.savetxt("./model/b_log.csv", self.b, delimiter = ",", fmt = "%s")
